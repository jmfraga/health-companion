...
3. **Personal clinical archive** as a user-facing surface — the visible side of the Phase-1 Supabase Storage migration. Users upload and keep their labs, imaging reports, specialist notes, hospitalization summaries, vaccination cards, prescriptions, audio notes from doctor visits, scale and BP photos. Searchable, filterable by date / type / source clinic, exportable. The companion ingests every upload into the longitudinal record and can recall *"the lipid panel from January 2026"* by name — but the user also has a documents view they own, on their device, in their pocket. Especially valuable in countries like México where clinical records are fragmented across providers with no national EHR, and the patient is the only one carrying the full file. Fulfills the *"your data is yours"* design principle from the README in a concretely visible way. *(First-viewer feedback on the submission video, 2026-04-26 — they specifically liked the idea of the product as a personal clinical repository.)*
4. **Settings surface lands**: language override, section-hide toggles (Vaccines optional), reasoning-visibility toggle, privacy export/delete, profile photo.
5. **Screening cards grow a "Set a date" affordance** → reminder against `reminders` → surfaces in Next Steps.
6. **Vaccines section** as a first-class layer (past + future, cited CDC ACIP / SSA México, hideable). Records pulled from the personal clinical archive (#3) where vaccination cards have been uploaded.
7. **Women's health as a first-class surface** — menstrual cycle tracking, fertility / pregnancy planning, prenatal continuity, postpartum, perimenopause, menopause. Today's prompt covers cervical screening (Pap / HPV) and breast (mammography, BRCA, NCCN early-start), but the lifelong arc of female reproductive and hormonal health deserves its own surface, not a subset of "preventive screening." Estrogen status shapes cardiovascular timing, bone density, mental-health screens, sleep architecture, and weight regulation — the whole prevention map shifts when this layer is honored. Pulls cycle / ovulation / pregnancy signals from Apple Health and Fitbit when the wearable integrations land. *(First-viewer feedback on the submission video, 2026-04-26 — flagged as a real gap, not a nice-to-have.)*
8. **Behavioral follow-through layer** — contextual reminders with emotional register, active verification, real celebration on follow-through.
9. **Wearable pilot**: Garmin + Apple Health + Google Fit / Health Connect. Start with HRV, RHR, nocturnal HR, respiratory rate, sleep, stress, body battery, SpO₂. Pair with the women's-health surface (#7) to ingest cycle / ovulation data where the wearable supports it.
10. **Proactivity v1 on Claude Managed Agents** in production (we already have the endpoint shape from the hackathon).
